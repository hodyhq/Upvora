...
func TestReactRenderer_RenderEmptyHomeHTML(t *testing.T) {
	...
	Expect(html).ContainsSubstring(`p-home__cta-btn`)
	Expect(html).ContainsSubstring(`Have an idea?`)
	...
	Expect(html).ContainsSubstring(`Powered by Upvora`)
	Expect(err).IsNil()
}
...
func TestReactRenderer_RenderEmptyHomeHTML_Portuguese(t *testing.T) {
	...
	Expect(html).ContainsSubstring(`<div class="c-dev-banner">DEV</div>`)
	Expect(html).ContainsSubstring(`p-home__cta-btn`)
	Expect(html).ContainsSubstring(`p-home__panel-title`)
