...
SELECT id, 'requesting_team',   'Requesting team',   'intake',         'text',   0, 'Which team or department is asking for this?',                          110, FALSE FROM tenants
UNION ALL
SELECT id, 'requested_by',      'Requested by',      'intake',         'user',   0, 'Who raised or sponsors this request?',                                  120, FALSE FROM tenants
UNION ALL
SELECT id, 'problem_statement', 'Problem statement', 'context',        'note',   0, 'What problem does this solve, and what happens if we do nothing?',      210, FALSE FROM tenants
UNION ALL
SELECT id, 'reference_link',    'Reference link',    'context',        'url',    0, 'Link to a doc, mockup, or example that explains the idea.',             220, FALSE FROM tenants
UNION ALL
SELECT id, 'process_affected',  'Process affected',  'workflow',       'multiline', 0, 'Which process or workflow would this change, and how is it handled today?', 310, FALSE FROM tenants
UNION ALL
SELECT id, 'needed_by',         'Needed by',         'workflow',       'date',   0, 'When does this need to be in place to matter?',                         320, FALSE FROM tenants
UNION ALL
SELECT id, 'business_owner',    'Business owner',    'ownership',      'user',   0, 'Who owns this after it ships?',                                         410, FALSE FROM tenants
UNION ALL
SELECT id, 'people_impacted',   'People impacted',   'classification', 'number', 0, 'Roughly how many people would use or benefit from this?',              520, FALSE FROM tenants
UNION ALL
SELECT id, 'decision_notes',    'Decision notes',    'decision',       'note',   0, 'Rationale, conditions, and next steps for the decision.',              910, FALSE FROM tenants
UNION ALL
SELECT id, 'next_review',       'Next review',       'decision',       'date',   0, 'When should this decision be revisited?',                               920, FALSE FROM tenants
